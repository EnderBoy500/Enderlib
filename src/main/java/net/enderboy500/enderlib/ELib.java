...
public final class ELib {
    ...
    public void addBuiltInDatapack(String name, String modId, PackActivationType packActivationType) {
        FabricLoader.getInstance().getModContainer(modId).ifPresent(modContainer -> ResourceLoaderImpl.registerBuiltinPack(Identifier.of(modId, name), "datapacks/" + Identifier.of(modId, name).getPath(), modContainer, packActivationType));
    }

    public void addBuiltInResourcepack(String name, String modId, PackActivationType packActivationType) {
        ResourceLoader.registerBuiltinPack(Identifier.of(name, modId), FabricLoader.getInstance().getModContainer(modId).get(), packActivationType);
    }
}
